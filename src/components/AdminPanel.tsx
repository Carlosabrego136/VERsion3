...
export default function AdminPanel() {
  ...
  if (!currentEventId) {
    ...
    disabled = { loading }
    className = "w-full py-4 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-white rounded-xl text-lg font-semibold shadow-lg shadow-amber-500/25 transition-all hover:shadow-xl hover:shadow-amber-500/30 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed"
      >
  ...
  {/* Step 1: Event Info */ }
{
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4" >
        <h2 className="text-xl font-bold text-gray-900" > Informacion del Evento < /h2>
          ...
          <label className="block text-sm font-medium text-gray-700 mb-1" > Nombre del Evento < /label>
            < input value = { event.name } onChange = { e => updateEvent({ name: e.target.value })
} placeholder = "Ej: XV de Valentina" className = "w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-400" />
  </div>
  ...
  <label className="block text-sm font-medium text-gray-700 mb-1" > Tipo de Evento < /label>
    < select value = { event.eventType } onChange = { e => updateEvent({ eventType: e.target.value })} className = "w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-400" >
    {
      ...
      < input type = "date" value = { event.eventDate } onChange = { e => updateEvent({ eventDate: e.target.value })
...
{/* Step 3: Guest Import */ }
{
  step === 3 && (
    <div className="max-w-2xl mx-auto space-y-4" >
      <GuestImport eventId={ event.id } onGuestsChanged = {() => setGuestRefreshKey(k => k + 1)
} key = { guestRefreshKey } />
  <div className="flex gap-3" >
